...
- Tag releases you want git checkouts to land on (`vYYYY.M-R` for stable, `vYYYY.M-R-beta.N` for beta).
- `package.json` stays semver-compatible as `YYYY.M.R` or `YYYY.M.R-beta.N`; tags use the public hyphenated form.
- Legacy OpenClaw tags are still part of repo history, but new MindClaw tags should follow the monthly release-number scheme.
