...
import { registerMemorySystemContextEngine } from "./memory-system.js";
import {
...
describe("Default engine selection", () => {
  ...
  beforeEach(() => {
    ...
    registerMemorySystemContextEngine();
  ...
  it("resolveContextEngine() with no config returns the default ('memory-system') engine", async () => {
    const engine = await resolveContextEngine();
    expect(engine.info.id).toBe("memory-system");
  });
