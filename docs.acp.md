...
This document describes how the MindClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`mindclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
MindClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
`mindclaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It is designed to route IDE prompts into an existing MindClaw Gateway
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the MindClaw gateway or agent instead.                                                                                                                                     |
...
drive a MindClaw Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `mindclaw acp` over stdio.

Example config:

```bash
mindclaw config set gateway.remote.url wss://gateway-host:18789
mindclaw config set gateway.remote.token <token>
```

Example run:

```bash
mindclaw acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
mindclaw acp --session agent:main:main
mindclaw acp --session agent:design:main
mindclaw acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “MindClaw ACP” to start a thread.

## Execution Model

- ACP client spawns `mindclaw acp` and speaks ACP messages over stdio.
...
```bash
mindclaw acp --session agent:main:main
mindclaw acp --session-label "support inbox"
mindclaw acp --reset-session
```
...
`mindclaw acp` resolves the Gateway URL and auth from CLI flags or config:
